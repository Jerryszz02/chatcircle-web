...
//   （邮箱不存在时不触发，PB 自身即返回 204 不发信）；
// - e.request 为 undefined，可用 e.realIP() / e.requestInfo() / e.json / e.noContent / e.collection / e.record；
// - return e.json(status, body) 短路请求（保持 { code, message, data: { code } } 统一错误形态）；
// - 不调用 e.next() 即静默取消后续处理（用于找回门控，配合 e.noContent(204) 保持与正常
//   响应一致的 204，不泄露账号是否存在/是否已验证）。
//
// 规则（窗口与数值见 security-privacy §9）：
// - per-email 频率限制：每端点每邮箱 1 小时滑窗内 3 次请求，超限 429 TOO_MANY_ATTEMPTS；
// - per-IP 频率限制：三端点共享一桶，1 小时滑窗内 20 次请求，超限 429 TOO_MANY_ATTEMPTS；
// - 找回门控：record.verified = false 时不调 e.next()、返回 204 静默拦截（不发邮件、不放行），
...
// ---------------------------------------------------------------------------
// 共享限流工具：三端点 per-email（按用途分桶）+ per-IP（三端点共桶）
// JSVM 各钩子作用域隔离，函数在每个钩子内就地展开（勿提取跨钩子共享）
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 找回密码：限流 + 「仅已验证邮箱可找回」门控（AC-24，静默 204 防账号枚举）
// ---------------------------------------------------------------------------
onRecordRequestPasswordResetRequest((e) => {
  ...
  if (record && !record.get('verified')) {
    writeAudit($app, {
      ...
      action: 'auth.password_reset.suppressed',
      targetType: 'admin_account',
      targetId: record.id,
      result: 'failure',
      reason: '邮箱未完成验证，找回密码请求被拦截',
    });
    e.noContent(204);
    return;
  }
