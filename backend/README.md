...
一键自举临时 PocketBase 实例（临时数据目录 → 全部迁移 → 测试超管 → 模板 SQL fixture → 显式三目录参数 serve），执行 `tests/integration/` 下全部套件：主链路 43 项、越权矩阵（AC-03）、名额硬校验与并发审核（AC-08）、状态迁移矩阵（AC-07）、签到唯一/幂等/补签撤销（AC-09/10/20）、问卷四条件与答卷生命周期（AC-11/12）、导出敏感过滤与开关（AC-16/17）、登录限流与邀请码（AC-21/02）、备份告警（AC-23）、无硬删除（AC-18）。输出逐条 PASS/FAIL 与汇总，任一失败退出码为 1。仅依赖 python3 标准库；端口可用 `CC_IT_PORT` 覆盖（默认 8097），`CC_IT_KEEP=1` 保留临时目录调试。详见 `tests/README.md`。
