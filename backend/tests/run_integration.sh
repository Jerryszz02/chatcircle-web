...
#   1. 准备二进制（缺失则按平台自动下载 0.39.7，与 migration_smoke.sh 同逻辑）
...
# Snapshot code before starting PocketBase. Its JSVM watches hooks and can
# restart the server when another local worker edits the shared checkout.
...
"$PB" migrate up --dir "$DATA_DIR" --migrationsDir "$MIGRATIONS_SNAPSHOT" --hooksDir "$HOOKS_SNAPSHOT" \
...
"$PB" serve --dir "$DATA_DIR" --migrationsDir "$MIGRATIONS_SNAPSHOT" --hooksDir "$HOOKS_SNAPSHOT" \
...
# --- 6. 执行全部套件 --------------------------------------------------------------
CC_IT_SERVE_LOG="$WORK/serve.log" python3 "$SCRIPT_DIR/integration/run.py" --base-url "$BASE" --su "$SU_EMAIL" --sp "$SU_PASS"
