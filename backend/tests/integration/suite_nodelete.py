...
断言：全部 19 个业务集合的 delete 通道对机构管理员与参与者关闭（403/404）；
...
def run(ctx):
    ...
    targets = [
        ...
        ('checkins', checkin), ('survey_templates', ctx['tpl_id']),
    ...
    rep.check('NDEL-00 fixture：19 个集合均有真实记录', all(t[1] for t in targets),
    ...
    rep.check('NDEL-20 参与者 delete 本人报名 → 403/404', s in (403, 404), r)
    s, r = call(base, 'DELETE', '/api/collections/participant_accounts/records/%s' % P, token=PT)
    rep.check('NDEL-21 参与者 delete 本人账号 → 403/404', s in (403, 404), r)
    ...
    s, r = call(base, 'POST', '/api/collections/audit_logs/records',
                {'actor_id': 'x', 'actor_role': 'admin', 'organization_id': org,
                 ...
                 'result': 'success'}, AT)
    rep.check('NDEL-22 管理员 create audit_logs → 403/404', s in (403, 404), r)
    s, r = call(base, 'PATCH', '/api/collections/audit_logs/records/%s' % audit,
                {'action': 'tampered'}, AT)
    rep.check('NDEL-23 管理员 update audit_logs → 403/404', s in (403, 404), r)
