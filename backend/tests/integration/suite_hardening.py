...
本套件依赖「到达第 3 节时，本次运行累计内置 auth-with-password 尝试 ≤14 次」。
当前全套件用量：super_login 1 + 各套件管理员登录 13 = 14（suite_nodelete 第二管理员
刻意超管直建不登录）。本套件在第 3 节前只做 1 次管理员登录（fixture），限流用例
固定为 5×400 + 第 6 次 429（第 6 次同时为 per-IP 第 21 次，双重命中，断言仍成立）。
新增套件时：不要再增加内置 auth 调用，否则本套件限流断言会提前被 per-IP 桶拦截；
本套件必须排在 runner 最后执行。
"""
import csv
import io
