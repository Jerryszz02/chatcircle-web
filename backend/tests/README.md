...
| `suite_acl.py` | **越权矩阵 AC-03**（test-plan §4）：机构A管理员→机构B 活动/报名/答案/签到/场次/问卷/题目/答卷/答案/导出/审计/机构/账号/培训/培训签到/培训场次的列表+详情，全部自定义端点，导出 scope 与看板参数伪造，横向：管理员→超管端点、参与者→管理端点与他人记录、未认证（含原生 view 收敛 404）、posts 推文越权写与 hidden 可见性（2026-08 改版） | 93 |
...
| `suite_nodelete.py` | 无硬删除 AC-18：23 个业务集合 delete 对管理员/参与者全拒、审计无创建/修改途径 | 28 |
