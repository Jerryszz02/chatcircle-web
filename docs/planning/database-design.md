...
> **2026-08-29 T2/T3 更新：**默认分支的 T3 已提供兼容当前 schema 的事务快照与 Realtime 守卫；本 T2 分支已实现并验证 §6.3/§6.4 的现场编号字段与 `activity_pairs`。手机号字段、标准字段激活和细粒度 `export_jobs.scope_json` 仍为`计划中`；本 T2 分支尚未合并或部署。
...
- 给出 PRD §9.1 全部 19 个集合的 PocketBase collection 定义草案；后续已新增培训三集合、reports、posts，以及 T2 `activity_pairs`，本分支合计 25 个业务集合。
