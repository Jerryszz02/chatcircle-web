...
> **2026-08-29 T2/T3 更新：**默认分支的 T3 单活动实时数据服务已验证，包括事务快照、双完成率、互补桶联合抑制与 Realtime 失效化/重连；本分支的 T2 现场编号与配对后端也已实现并验证，尚未合并或部署。手机号、工作台/参与者 UI 与细粒度导出仍`计划中`，共享契约继续使用 `2026-08-28.t0-v1`。
