...
> **2026-08-29 T1/T2/T3 更新：**参与者手机号账号、Dypnsapi 验证码、存量绑定与换绑，T2 现场编号与配对后端，以及单活动事务快照、双完成率、互补桶联合抑制和 Realtime 失效化/重连均`已验证`；真实阿里云账号联调仍为上线门禁。T4 机构活动工作台（创建向导、复制活动、生命周期首页、现场工作台与配对管理）已验证并合并默认分支（#41）；T5 参与者配对体验（三入口共用 my-pairing 快照与本人 Realtime 失效事件）在 `agent/t5-participant-pairing` `已验证`，尚未合并或部署；细粒度导出（T6）仍为`计划中`；共享契约保持 `2026-08-28.t0-v1`。
...
| 仓库现状 | 项目根目录，2026-08-30 | 默认分支已包含 T1 手机号认证、T2 现场编号/配对、T3 快照/Realtime 与 T4 机构活动工作台（#36~#41 均已合并）；T5 参与者配对体验在 `agent/t5-participant-pairing` 已验证、尚未合并或部署；细粒度导出仍未实现。本文主体源于开发前设计，代码现状与本文冲突时以代码和 [开发者指南](../developer-guide.md) 为准 |
