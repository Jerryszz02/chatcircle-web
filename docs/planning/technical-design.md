...
| 端 | 使用者 | 首要设备 | 职责 |
| --- | --- | --- | --- |
| 参与者端 | 倾诉者 / 聆听者 | 手机（360px 宽无横向滚动，PRD §13） | 公开活动详情、报名链路内自动注册/登录、通用登录页、提交报名、"我的"中心（报名状态、问卷入口、已提交答案）、扫码签到、问卷填写与只读答案查看、报名成功页回访指引 |
...
- 三种会话（`participant_accounts` / `admin_accounts` / PocketBase `_superusers`）互不通用；路由守卫按认证记录类型分流，参与者无任何进入管理后台的入口（PRD §3.2）。
- 路由守卫只是 UX 引导；真正的隔离在 collection API rules 与 hooks（见 §5.5）。
- 活动与问卷不出现在任何公开列表，仅链接/二维码可达（FR-ACT-002）；V1 无公开活动广场。
