...
1. 生产短信 provider、隐私版本、必填环境变量占位、TLS/安全头、管理台封闭与部署预检的静态配置检查；
...
- [ ] 生产 HTTPS、HSTS、CSP、`/_/` 403、证书续期与真实来源 IP 记录均正常；不再需要的服务器 `docker-compose.override.yml` 已删除。
- [ ] RAM 权限已收窄到短信认证和指定 DNS hosted zone 所需最小 Action；两组凭据可分别轮换。
