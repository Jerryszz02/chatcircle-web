...
>
> **T1/T3 边界（2026-08-29）**：`frontend/src/shared/api/accountEvent.ts` 的手机号契约已由 migration、`phoneauth.pb.js` 和参与者 UI 落地；单活动实时数据服务也已实现。现场配对、工作台 UI 与细粒度导出仍未实现。该结论只代表当前分支，不代表已合并或已部署。
...
│   ├── pb_migrations/   #   版本化 schema（27 个迁移文件 / 25 个业务或内部集合），schema 变更的唯一入口
│   ├── pb_hooks/        #   服务端业务规则（JSVM *.pb.js）：50 个自定义路由 + Realtime 守卫 + 写守卫 + 审计
...
4. **无硬删除**。全部 25 个业务/内部集合 `deleteRule: null`；停用/归档/作废/撤销一律用 `status` 字段表达（FR-AUD-001）。改代码时不要引入任何删除语义。
...
### 5.4 自定义端点全集（50 个 `routerAdd`，统一 `/api/cc/*` 前缀）
