 ...
 * reason, metadata } | null, alert: boolean, stale?: boolean, message? }
 * （尚无备份时 last_backup=null 且 alert=true；最近一次成功超过 36h 未刷新时
 * stale=true 且 alert=true，message 携带中断说明）。此处仍保留防御式归一化，
 * 兼容 alarm/failed 等历史命名：
 * - alarm：读取 alert / alarm / failed 字段，缺失时按最近一次备份结果推断（failure → 告警）；
 * - stale：读取 stale 字段（陈旧成功掩盖中断防护，见 super.pb.js backup-status）；
 * - last_backup：最近一次备份的时间/结果/文件/失败原因，字段名兼容常见命名。
 */

/** 归一化后的备份状态（页面只依赖此结构）。 */
export interface BackupStatus {
  /** 是否应显示显著告警横幅（最近一次备份失败/中断/无记录）。 */
  alarm: boolean;
  /** 最近一次成功是否已陈旧（超过每日周期宽限未刷新，任务疑似中断）。 */
  stale: boolean;
  /** 最近一次备份信息；从未备份为 null。 */
  lastBackup: {
    at?: string;
...
export function normalizeBackupStatus(raw: unknown): BackupStatus {
  if (typeof raw !== 'object' || raw === null) {
    return { alarm: false, stale: false, lastBackup: null };
  }
  ...
  return {
    alarm,
    stale: typeof obj.stale === 'boolean' ? obj.stale : false,
    lastBackup: last
      ? {
          at: pickString(last, ['at', 'time', 'created', 'finished_at']),
          result,
          file: pickString(last, ['file', 'file_name', 'filename']),
          // 中断告警的说明在顶层 message（last_backup.reason 为空），顶层兜底
          message:
            pickString(last, ['message', 'error', 'reason']) ?? pickString(obj, ['message']),
        }
      : null,
  };
}
